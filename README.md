...
In this folder, there is a template project for inference of trained, exported models of YoloV3 on OpenVino. In the following
...
6. Copy all exported YoloV3 models to ```./exported-models```. The folder names look like this ```pt_yolov3s_640x360_oxfordpets_e300``` according to the interface from 
...
```convert_yolo_onnx_to_ir_pt_yolov3s_640x360_oxfordpets_e300.sh```, where the MODELNAME is a exact match of the folder name of the model in ```exported-models```.
